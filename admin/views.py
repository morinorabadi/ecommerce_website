...
"""
you can use below code to chech is authenticated

import code
from admin.views import is_authenticated

chech code
admin = is_authenticated(request)
if type(admin) == Response : return admin 
"""


# for create new employe model
@api_view(("POST",))
def signup(request):
    """
    return Response with empoye data if success
    """
    admin = is_authenticated(request)
    ...
    data = request.data
    data["boss"] = admin
    instance = serializers.AdminSignUp(data=data)
    if instance.is_valid(True):
        instance.save()
        return Response(instance.data, status=status.HTTP_200_OK)


# this can be used for admin login
@api_view(("POST",))
def login(request):
    """
    return Response with cookie if success
    """
    ...
    try:
        instance = models.Employe.objects.get(username=username)
        if not instance.password == password:
            raise ValueError
    except (models.Employe.DoesNotExist, ValueError) as error:
    ...
    data = serializers.AdminSignUp(instance)
...
"""
 ...
 but client still can send request to apis but
 apis are secure with is_authenticated() method
"""
...
def is_authenticated_api(request):
    ...
    if type(admin) is Response:
        admin
    return Response({"msg": "is authenticated"}, status=status.HTTP_200_OK)
...
def is_authenticated(request):
    """
    return "employe model" if is_authenticated else return "Response"
    """
    ...
    if datetime.timedelta(minutes=30) < time_now - moritoken.time.replace(tzinfo=None):
        moritoken.delete()
